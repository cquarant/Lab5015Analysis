...
int main(int argc, char** argv) {
	...
	std::map<double, TProfile*> p1_tR_REFvsER_corr;
	std::map<double, TH2F*> h2_tAve_REFvsE;
	std::map<double, TProfile*> p1_tAve_REFvsE;
	std::map<double, TH2F*> h2_tAve_REFvsE_corr;
	std::map<double, TProfile*> p1_tAve_REFvsE_corr;
	...
	std::map<double, TH1F*> h1_deltaT_ave_corr_REF;
	std::map<double, TH1F*> h1_deltaT_LR_corr_REF;
	std::map<double, TH1F*> h1_deltaT_LR_corr_REF_w;

	// --- external bar
    std::map<int, TH1F*> h1_energyRatio_REF;
	std::map<int, TH2F*> h2_energyLvsR_REF;
	std::map<int, TH1F*> h1_energyL_ext;
	std::map<int, TH1F*> h1_energyR_ext;
	std::map<int, TH1F*> h1_energyLR_ext;
	std::map<int, TProfile*> p1_energyLvsR_REF;
	...
	// order bar list and get max bar number
	std::sort(barList.begin(), barList.end());
	int maxBar = barList.back();
    ...
    for(auto stepLabel : stepLabels) {   
       ...
       if(useTrackInfo && !opts.GetOpt<std::string>("Coincidence.status").compare("yes") ){
        ...
        for(int iBar = 0; iBar < 16; ++iBar) {
	        ...
	        for(auto LRLabel : LRLabels ) {
				...
				for(auto PPLabel : PPLabels){
					if(PPLabel == "PREPOST") continue;
					if(LRLabel == "L-R") continue; 
					std::string label_sum(Form("%s_bar%02d%s_%s",PPLabel.c_str(),iBar,LRLabel.c_str(),stepLabel.c_str()));
				    std::string label_single(Form("%s_ONLY_bar%02d%s_%s",PPLabel.c_str(),iBar,LRLabel.c_str(),stepLabel.c_str()));

					c = new TCanvas(Form("c_energy_%s",label_sum.c_str()),Form("c_energy_%s",label_sum.c_str()));
	                gPad -> SetLogy();
	  
	            	histo = (TH1F*)( inFile->Get(Form("h1_energy_%s",label_sum.c_str())) );      
	            	if( !histo ){
						std::cout<<"h1_energy_"<<label_sum<<" not found"<<std::endl;
						continue;
					}
	                latex = new TLatex(0.40,0.85,Form("#splitline{%s bar %02d%s sum }{V_{OV} = %.2f V, th. = %d DAC}",PPLabel.c_str(),iBar,LRLabel.c_str(),Vov,int(vth1)));
                    latex -> SetNDC();
	            	latex -> SetTextFont(42);
	            	latex -> SetTextSize(0.04);
	            	latex -> SetTextColor(kRed);
	                ...
	                outFile -> cd();
	                histo->Write();     
	                c -> Print(Form("%s/energy/ch%s_double/c_energy__%s.png",plotDir.c_str(),LRLabel.c_str(),label_sum.c_str()));
	                c -> Print(Form("%s/energy/ch%s_double/c_energy__%s.pdf",plotDir.c_str(),LRLabel.c_str(),label_sum.c_str()));
	                delete c;
	                delete latex;

					c = new TCanvas(Form("c_energy_%s",label_single.c_str()),Form("c_energy_%s",label_single.c_str()));
	                gPad -> SetLogy();
	  
	            	histo = (TH1F*)( inFile->Get(Form("h1_energy_%s",label_single.c_str())) );      
	            	if( !histo ){
						std::cout<<"h1_energy_"<<label_single<<" not found"<<std::endl;
						continue;
					}
	                histo->GetXaxis()->SetRangeUser(0,1024);
	                latex = new TLatex(0.40,0.85,Form("#splitline{%s bar %02d%s single }{V_{OV} = %.2f V, th. = %d DAC}",PPLabel.c_str(),iBar,LRLabel.c_str(),Vov,int(vth1)));
                    latex -> SetNDC();
	            	latex -> SetTextFont(42);
	            	latex -> SetTextSize(0.04);
	            	latex -> SetTextColor(kRed);
	                ...
	                outFile -> cd();
	                histo->Write();  
	                c -> Print(Form("%s/energy/ch%s_double/c_energy__%s.png",plotDir.c_str(),LRLabel.c_str(),label_single.c_str()));
	                c -> Print(Form("%s/energy/ch%s_double/c_energy__%s.pdf",plotDir.c_str(),LRLabel.c_str(),label_single.c_str()));
	                delete c;
	                delete latex;
			    }
			...
			if(!histo1){
				std::cout<<Form("h1_energy_%s",label.c_str()) <<" not found"<<std::endl;
			    continue;
			}
			...
			if(!histo2){
				std::cout<<Form("h1_energy_PRE_%s",label.c_str()) <<" not found"<<std::endl;
				continue;
			}
			histo3 = (TH1F*)(inFile->Get( Form("h1_energy_POST_%s",label.c_str()) ) );
			if(!histo3){
				std::cout<<Form("h1_energy_POST_%s",label.c_str()) <<" not found"<<std::endl;
				continue;
			}
			histo3 -> GetXaxis()->SetRangeUser(0,2024);
			histo4 = (TH1F*)(inFile->Get( Form("h1_energy_PREPOST_%s",label.c_str()) ) );
			if(!histo4){
				std::cout<<Form("h1_energy_PREPOST_%s",label.c_str()) <<" not found"<<std::endl;
				continue;
			}
            
            histo_sum = (TH1F*)(histo1->Clone(Form("histo_sum_%s",label.c_str() )  ) );
			histo_sum->Add(histo2);
			histo_sum->Add(histo3);
			histo_sum->Add(histo4);
	        ...
	        histo_sum -> Draw();
			histo_sum->GetXaxis()->SetRangeUser(minE[std::make_pair(iBar, Vov)], 1600);
			//-----start fit-----
			float max = histo_sum->GetBinCenter(histo_sum->GetMaximumBin());
			histo_sum->GetXaxis()->SetRangeUser(0, 2024);
			...
			f_landau1[index] -> SetParameters(histo_sum->Integral(histo_sum->GetMaximumBin(), histo_sum->GetNbinsX())/10, max, 0.1*max);
	        f_landau1[index] -> SetParLimits(1,0,9999);
	        f_landau1[index] -> SetParLimits(2,0,9999);
            histo_sum->Fit(f_landau1[index],"QRS");
			if ( f_landau1[index]->GetParameter(1) > 0 ){
				xmin = f_landau1[index]->GetParameter(1) - 2 * std::abs(f_landau1[index]->GetParameter(2));
				if (xmin < minE[std::make_pair(iBar, Vov)]) xmin = minE[std::make_pair(iBar, Vov)] ;
				xmax = std::min(f_landau1[index]->GetParameter(1) * 2.5, 1600.);
				f_landau1[index] -> SetRange(xmin, xmax);
				f_landau1[index] -> SetParameters(histo_sum->Integral(histo_sum->GetMaximumBin(), histo_sum->GetNbinsX())/10, f_landau1[index]->GetParameter(1), 0.1*f_landau1[index]->GetParameter(1));
			}
			histo_sum->Fit(f_landau1[index],"QRS");
			f_landau1[index] -> SetLineColor(kBlack);
	        f_landau1[index] -> SetLineWidth(2);
	        f_landau1[index] -> Draw("same");
			//std::cout<<f_landau1[index]->GetParameter(0)<<"  "<<f_landau1[index]->GetParameter(1)<<"  "<<std::abs(f_landau1[index]->GetParameter(2))<<std::endl;
			ranges_all[index]->push_back( f_landau1[index]->GetParameter(1) - 2.0 * std::abs(f_landau1[index]->GetParameter(2)));
			ranges_all[index]->push_back(std::min(f_landau1[index]->GetParameter(1)*2.5, 1600.));
            //std::cout<<" "<<std::endl;
			...
			histo_sum -> Write();
			c->Update();
			c -> Print(Form("%s/energy/A_energySum/c_energy_sum_%s.png",plotDir.c_str(),label.c_str()));
	        c -> Print(Form("%s/energy/A_energySum/c_energy_sum_%s.pdf",plotDir.c_str(),label.c_str()));
	        delete c;

			legend = new TLegend(0.73, 0.5, 0.98, 0.68);
			histo1->SetLineColor(kYellow);
			histo1->GetListOfFunctions()->Clear();
            ...
            legend->AddEntry(histo2, Form("bars (%02d + %02d)",iBar-1,iBar), "f");
            legend->AddEntry(histo3, Form("bars (%02d + %02d)",iBar,iBar+1), "f");
            legend->AddEntry(histo4, Form("bars (%02d + %02d + %02d)",iBar-1,iBar,iBar+1), "f");
            legend->Draw();

			c->Update();
			c -> Print(Form("%s/energy/A_energySum/c_energy_sum_%s_1.png",plotDir.c_str(),label.c_str()));
	        c -> Print(Form("%s/energy/A_energySum/c_energy_sum_%s_1.pdf",plotDir.c_str(),label.c_str()));
        ...
        c = new TCanvas(Form("c_adjacent_bars_%s",stepLabel.c_str()),Form("c_adjacent_bars_%s",stepLabel.c_str()));
	    ...
	    latex -> SetTextSize(0.04);
	    latex -> SetTextColor(kRed);
		histo -> SetTitle(";nHits;events");
	    histo -> SetLineColor(kRed);
	    histo -> SetLineWidth(2);
		histo -> GetYaxis()->SetRangeUser(0, histo->GetMaximum()*1.1);
	    histo -> Draw();
		latex -> Draw();
		c -> Print(Form("%s/topology/c_adjacent_bars__%s.png",plotDir.c_str(),label.c_str()));
	    c -> Print(Form("%s/topology/c_adjacent_bars__%s.pdf",plotDir.c_str(),label.c_str()));
        delete c;

		c = new TCanvas(Form("c_counter_%s",stepLabel.c_str()),Form("c_counter_%s",stepLabel.c_str()));
		TH1F* h_temp = (TH1F*) histo->Clone(Form("h_counter_freq_%s",stepLabel.c_str()));
		if (h_temp->Integral() > 0) {
			h_temp->Scale(1.0 / h_temp->Integral());
		}
        h_temp->GetYaxis()->SetRangeUser(0, 1);
	    h_temp -> SetTitle(";nHits;frequency");
		...
		c -> Print(Form("%s/topology/c_counter__%s.png",plotDir.c_str(),label.c_str()));
	    c -> Print(Form("%s/topology/c_counter__%s.pdf",plotDir.c_str(),label.c_str()));
	    delete c;
	    delete latex;

        TH1F* histo1_3bins = (TH1F*)( inFile->Get(Form("h1_events_type3_%s",label.c_str())) );
		c = new TCanvas(Form("c_events_type_%s",stepLabel.c_str()),Form("c_events_type_%s",stepLabel.c_str()));
		histo1_3bins -> SetTitle(";nHits;events");
	    histo1_3bins -> SetLineColor(kBlue);
	    histo1_3bins -> SetLineWidth(2);
		//histo1_3bins -> GetXaxis()->SetRangeUser(1.5,3.5);
		histo1_3bins -> GetYaxis()->SetRangeUser(0, histo1_3bins->GetMaximum()*1.1);
	    histo1_3bins -> Draw();
		histo -> Draw("same");
	    ...
	    latex -> SetTextColor(kRed);
		latex->Draw();
		TLegend* legend = new TLegend(0.6,0.5,0.8,0.6);
		legend -> AddEntry(histo,"Adjacent Bars","l");
		legend -> AddEntry(histo1_3bins,"Active Bars","l");
		legend -> Draw();
		c -> Print(Form("%s/topology/c_events_type__%s.png",plotDir.c_str(),label.c_str()));
	    c -> Print(Form("%s/topology/c_events_type__%s.pdf",plotDir.c_str(),label.c_str()));
		delete c;
        
		
		c = new TCanvas(Form("c_nonAdjacent_bars_%s",stepLabel.c_str()),Form("c_nonAdjacent_bars_%s",stepLabel.c_str()));
		histo1_3bins->Add(histo,-1.0);
        histo1_3bins -> SetTitle(";nHits;events");
	    histo1_3bins -> SetLineColor(kBlue);
	    histo1_3bins -> SetLineWidth(2);
		histo1_3bins -> GetYaxis()->SetRangeUser(0, histo1_3bins->GetMaximum()*1.1);
	    histo1_3bins -> Draw();
		c -> Print(Form("%s/topology/c_nonAdjacent_bars__%s.png",plotDir.c_str(),label.c_str()));
	    c -> Print(Form("%s/topology/c_nonAdjacent_bars__%s.pdf",plotDir.c_str(),label.c_str()));
		delete c;

		c = new TCanvas(Form("c_nActiveBars_%s",stepLabel.c_str()),Form("c_nActiveBars_%s",stepLabel.c_str()));
		TH1F* histo1;
		histo1 = (TH1F*)( inFile->Get(Form("h1_events_type_%s",label.c_str())) );      
	    if( !histo1 ) continue;
		histo1 -> SetTitle(";nHits;events");
	    histo1 -> SetLineColor(kBlue);
	    histo1 -> SetLineWidth(2);
	    histo1 -> Draw();
		latex = new TLatex(0.40,0.85,Form("#splitline{number of active bars}{V_{OV} = %.2f V, th. = %d DAC}",Vov,int(vth1)));
		latex -> SetNDC();
	    latex -> SetTextFont(42);
	    latex -> SetTextSize(0.04);
	    latex -> SetTextColor(kRed);
		latex->Draw();
		c -> Print(Form("%s/topology/c_ActiveBars__%s.png",plotDir.c_str(),label.c_str()));
	    c -> Print(Form("%s/topology/c_ActiveBars__%s.pdf",plotDir.c_str(),label.c_str()));
		delete c;

		c = new TCanvas(Form("c_plus2ActiveBars_%s",stepLabel.c_str()),Form("c_plus2ActiveBars_%s",stepLabel.c_str()));
		//histo1-> GetXaxis()->SetRangeUser(1.5,16.5);
	    histo1 -> Draw();
		latex = new TLatex(0.40,0.85,Form("#splitline{number of active bars > 1}{V_{OV} = %.2f V, th. = %d DAC}",Vov,int(vth1)));
		latex -> SetNDC();
	    latex -> SetTextFont(42);
	    latex -> SetTextSize(0.04);
	    latex -> SetTextColor(kRed);
		latex->Draw();
		c -> Print(Form("%s/topology/c_plus2ActiveBars__%s.png",plotDir.c_str(),label.c_str()));
	    c -> Print(Form("%s/topology/c_plus2ActiveBars__%s.pdf",plotDir.c_str(),label.c_str()));
		delete c;
	...
	std::vector<float>* enRatioBins = new std::vector<float>({0.1,0.3,0.8,1.3,1.8,2.3}); 
    ...
    for(auto mapIt : trees){
        ...
        for(int entry = 0; entry < nEntries; ++entry){
			...
			int index0( (10000*int(anEvent->Vov*100.)) + (100*anEvent->vth1) + 99 );
	        int index1( (10000*int(anEvent->Vov*100.)) + (100*anEvent->vth1) + anEvent->barID );
	  
	        accept[index1][entry] = false;
			// at the moment the energyBinAverage value is used to apply selection on the energy range
			// defining a binned range, it is possible to divide events according to the deposited energy in the module
			int energyBinAverage=0;
			int enRatioBin=0;
            if(anEvent->nClusters<0) continue;
	        if(anEvent->nClusters==1) {
				...
				if(energyBinAverage < 1 ) continue; // single-event selection
			}
			else if(anEvent->nClusters==2 && anEvent->energyL_post>0 && anEvent->energyR_post>0){
				if(!ranges_doubleHits[index1] ) continue;
				
				// --- selection for energy ratio binning
                if(FindBin(anEvent->energySum,ranges_doubleHits[index1])<0) continue;
				float E_mean = 0.5*(anEvent->energyL + anEvent->energyR);
				float E_mean_post = 0.5*(anEvent->energyL_post + anEvent->energyR_post);
				float energyRatio = E_mean/E_mean_post;
				energyBinAverage = FindBin(anEvent->energySum,ranges_doubleHits[index1])+1;
				enRatioBin = FindBin(energyRatio, enRatioBins)+1;				
				if(enRatioBin < 1 ) continue;// remove events out of energy ratio bins
            ...
            double index2( (100000000*anEvent->nClusters) + 10000000*energyBinAverage + (double)index1 );
            double index3( 10000000000*enRatioBin + index2 );
	  
	        accept[index1][entry] = true;

			if(h1_energyRatio_REF[index0]==NULL){
				h1_energyRatio_REF[index0] = new TH1F(Form("h1_energyRatio_externalBar_Vov%.2f_th%02d",anEvent->Vov,anEvent->vth1),"",1000,0.,10.);
				h1_t1fineMean_REF[index0] = new TH1F(Form("h1_t1fineMean_externalBar_Vov%.2f_th%02d",anEvent->Vov,anEvent->vth1),"",1000,0.,1000.);
				h2_energyLvsR_REF[index0] = new TH2F(Form("h2_energyLvsR_externalBar_Vov%.2f_th%02d",anEvent->Vov,anEvent->vth1),"",512,0,1024,512,0,1024);
				p1_energyLvsR_REF[index0] = new TProfile(Form("p1_energyLvsR_externalBar_Vov%.2f_th%02d",anEvent->Vov,anEvent->vth1),"",512,0,1024);
				h1_energyL_ext[index0] = new TH1F(Form("h1_energyL_ext_Vov%.2f_th%02d",anEvent->Vov,anEvent->vth1),"",512,0,1024);
				h1_energyR_ext[index0] = new TH1F(Form("h1_energyR_ext_Vov%.2f_th%02d",anEvent->Vov,anEvent->vth1),"",512,0,1024);
				h1_energyLR_ext[index0] = new TH1F(Form("h1_energyLR_ext_Vov%.2f_th%02d",anEvent->Vov,anEvent->vth1),"",512,0,1024);
			...
			if(h2_energy_correlation[index2]==NULL){// energy correlation for double hits and triple hits
				std::string labelLR_energyBin(Form("bar%02dL-R_Vov%.2f_th%02d_energyBin%02d_%02dHits",anEvent->barID,anEvent->Vov,anEvent->vth1,energyBinAverage,anEvent->nClusters));
				...
				h2_energy_correlation3L[index2]= new TH2F(Form("h2_energy_correlation3L_%s",labelLR_energyBin.c_str()),"",500,0,2000,500,0,1000);
				p1_energy_correlation3L[index2]= new TProfile(Form("p1_energy_correleation3L_%s",labelLR_energyBin.c_str()),"",80,0,2000);
				h2_energy_correlation3R[index2]= new TH2F(Form("h2_energy_correlation3R_%s",labelLR_energyBin.c_str()),"",500,0,1000,500,0,2000);
				p1_energy_correlation3R[index2]= new TProfile(Form("p1_energy_correleation3R_%s",labelLR_energyBin.c_str()),"",40,0,1000);
			}

			if(h1_energyRatio_bars_F[index2]==NULL){// Energy ratio Correction plots for deltaT between two adjacent bars
	            ...
	            h2_deltaT_energyRatio_bars[index2] = new TH2F(Form("h2_detaT_energyRatio_%s",labelLR_energyBin.c_str()),"",100,0.,5.,100,-4000.,4000.);
				h2_deltaT_energyRatio_bars_w[index2] = new TH2F(Form("h2_detaT_energyRatio_w_%s",labelLR_energyBin.c_str()),"",100,0.,5.,100,-4000.,4000.); //2000 bins prima su asse y
			...
			if(h1_deltaT_bars[index3]==NULL){// study using deltaT between two adjacent bars (not final)
				std::string labelLR_energyBin(Form("bars%02d-%02d_Vov%.2f_th%02d_energyBin%02d_enRatioBin%02d_%02dHits",anEvent->barID,anEvent->barID+1,anEvent->Vov,anEvent->vth1,energyBinAverage,enRatioBin,anEvent->nClusters));
				h1_deltaT_bars[index3] = new TH1F(Form("h1_deltaT__%s",labelLR_energyBin.c_str()),"",2000,-24000.,24000.);//2000 bins prima
				h1_deltaT_bars_w[index3] = new TH1F(Form("h1_deltaT_w_%s",labelLR_energyBin.c_str()),"",2000,-24000.,24000.);
			}

			//////////////////////////////////////////////////////////////
			////// Begin Filling of 2nd loop histograms //////////////////
			//////////////////////////////////////////////////////////////

            // reference module 
			if (fabs(anEvent->timeR_ext-anEvent->timeL_ext)<10000 && anEvent->energyR_ext>0 && anEvent->energyL_ext>0){
				h1_energyRatio_REF[index0] -> Fill( anEvent->energyR_ext / anEvent->energyL_ext );
				h1_t1fineMean_REF[index0] -> Fill(0.5*(anEvent->t1fineR_ext + anEvent->t1fineL_ext));
				h2_energyLvsR_REF[index0] -> Fill(anEvent->energyL_ext,anEvent->energyR_ext);
				p1_energyLvsR_REF[index0] -> Fill(anEvent->energyL_ext,anEvent->energyR_ext);
				h1_energyL_ext[index0] -> Fill(anEvent->energyL_ext);
				h1_energyR_ext[index0] -> Fill(anEvent->energyR_ext);
				h1_energyLR_ext[index0] -> Fill(0.5*(anEvent->energyL_ext + anEvent->energyR_ext));
			}
			...
			if(anEvent->nClusters==2 && anEvent->energyL_post>0 && anEvent->energyR_post>0){
				if(anEvent->barID==maxBar) continue;
				...
				if(fabs(t_mean-t_mean_post)<10000){
					...
					/*if((anEvent->energyR/anEvent->energyL > energyRatio_ranges[index2]->at(0)) && (anEvent->energyR/anEvent->energyL < energyRatio_ranges[index2]->at(1) ) &&
						...
						// --- energy ratio between bar(i) and bar(i+1) and vice versa
			...
			if(anEvent->nClusters==3){
				if(anEvent->barID==0 || anEvent->barID==maxBar) continue;
				double E_mean=0.5*(anEvent->energyL+anEvent->energyR);
				double E_mean_post=0.5*(anEvent->energyL_post+anEvent->energyR_post); 
    ...
    for(auto stepLabel : stepLabels){
		...
		int index0( (10000*int(Vov*100.)) + (100*vth1) + 99 );
		...
		histo = h1_energyRatio_REF[index0];
		...
		fitFunc_energyRatio_REF[index0] = new TF1(Form("fitFunc_energyRatio_%s",extLabel.c_str()),"gaus",histo->GetMean()-2.*histo->GetRMS(),histo->GetMean()+2.*histo->GetRMS());
		histo -> Fit(fitFunc_energyRatio_REF[index0],"QNRS");
		histo -> Fit(fitFunc_energyRatio_REF[index0],"QSR+","",fitFunc_energyRatio_REF[index0]->GetParameter(1)-2.*fitFunc_energyRatio_REF[index0]->GetParameter(2),fitFunc_energyRatio_REF[index0]->GetParameter(1)+2.*fitFunc_energyRatio_REF[index0]->GetParameter(2));
		histo -> Fit(fitFunc_energyRatio_REF[index0],"QSR+","",fitFunc_energyRatio_REF[index0]->GetParameter(1)-2.*fitFunc_energyRatio_REF[index0]->GetParameter(2),fitFunc_energyRatio_REF[index0]->GetParameter(1)+2.*fitFunc_energyRatio_REF[index0]->GetParameter(2));
		  
		fitFunc_energyRatio_REF[index0] -> SetLineColor(kBlack);
		fitFunc_energyRatio_REF[index0] -> SetLineWidth(2);
		fitFunc_energyRatio_REF[index0] -> Draw("same");
		...
		histo = h1_t1fineMean_REF[index0];
		histo -> GetXaxis() -> SetRangeUser(histo->GetMean()-5.*histo->GetRMS(),histo->GetMean()+5.*histo->GetRMS());
		histo -> SetMaximum(1.25*histo->GetBinContent(histo->FindBin(FindXMaximum(histo,histo->GetMean()-2.*histo->GetRMS(),histo->GetMean()+2.*histo->GetRMS()))));
		...
		// -- draw energy correlation for external bar
		c = new TCanvas(Form("c_energyLvsR_%s",extLabel.c_str()),Form("c_energyLvsR_%s",extLabel.c_str()));
		h2 = h2_energyLvsR_REF[index0];
		h2 -> SetTitle(Form(";energy_{left};energy_{right}"));
		h2 -> SetLineColor(kRed);
		h2 -> SetLineWidth(2);
		h2 -> Draw("colz");
		prof = p1_energyLvsR_REF[index0];
		prof -> SetMarkerColor(kBlack);
		prof -> SetMarkerSize(0.4);
		prof -> Draw("psame");
		latex -> Draw("same");

		c -> Print(Form("%s/externalBar/c_energyLvsR__%s.png",plotDir.c_str(),extLabel.c_str()));
		c -> Print(Form("%s/externalBar/c_energyLvsR__%s.pdf",plotDir.c_str(),extLabel.c_str()));
		delete c;

		// -- draw energy for external bar
        c = new TCanvas(Form("c_energyLR_ext_%s",stepLabel.c_str()),Form("c_energyLR_ext_%s",stepLabel.c_str()));
		histo = h1_energyLR_ext[index0];
		gPad -> SetLogy();
		...
		histo -> Draw("");
        latex ->Draw("same");
		c -> Print(Form("%s/externalBar/c_energyLR_ext__%s.png",plotDir.c_str(),stepLabel.c_str()));
		c -> Print(Form("%s/externalBar/c_energyLR_ext__%s.pdf",plotDir.c_str(),stepLabel.c_str()));
		delete c;

		c = new TCanvas(Form("c_energyL_ext_%s",stepLabel.c_str()),Form("c_energyL_ext_%s",stepLabel.c_str()));
		histo = h1_energyL_ext[index0];
		gPad -> SetLogy();
		histo -> SetTitle(Form(";energy_{left} [a.u.];entries"));
		histo -> SetLineColor(kRed);
		histo -> SetLineWidth(2);
		histo -> Draw("");
		latex = new TLatex(0.40,0.85,Form("#splitline{external bar chL}{V_{OV} = %.2f V, th. = %d DAC}",Vov,int(vth1)));
		latex -> SetNDC();
		latex -> SetTextFont(42);
		latex -> SetTextSize(0.04);
		latex -> SetTextColor(kRed);
        latex -> Draw("same");
		c -> Print(Form("%s/externalBar/c_energyL_ext__%s.png",plotDir.c_str(),stepLabel.c_str()));
		c -> Print(Form("%s/externalBar/c_energyL_ext__%s.pdf",plotDir.c_str(),stepLabel.c_str()));
		delete c;

		c = new TCanvas(Form("c_energyR_ext_%s",stepLabel.c_str()),Form("c_energyR_ext_%s",stepLabel.c_str()));
		histo = h1_energyR_ext[index0];
		gPad -> SetLogy();
		histo -> SetTitle(Form(";energy_{right} [a.u.];entries"));
		histo -> SetLineColor(kRed);
		histo -> SetLineWidth(2);
		histo -> Draw("");
		latex = new TLatex(0.40,0.85,Form("#splitline{external bar chR}{V_{OV} = %.2f V, th. = %d DAC}",Vov,int(vth1)));
		latex -> SetNDC();
		latex -> SetTextFont(42);
		latex -> SetTextSize(0.04);
		latex -> SetTextColor(kRed);
        latex -> Draw("same");
		c -> Print(Form("%s/externalBar/c_energyR_ext__%s.png",plotDir.c_str(),stepLabel.c_str()));
		c -> Print(Form("%s/externalBar/c_energyR_ext__%s.pdf",plotDir.c_str(),stepLabel.c_str()));
		delete c;
		delete latex;
		std::cout << " drawn external bar plots for Vov = "<<Vov<<" V, th. = "<<int(vth1)<<" DAC "<<std::endl;

        for(int iBar = 0; iBar < 16; ++iBar) {
			std::cout << " Processing bar " << iBar << std::endl;
	        bool barFound = std::find(barList.begin(), barList.end(), iBar) != barList.end() ;
            if (!barFound) continue;      
				...
				for(int iEnergyBin = 1; iEnergyBin <= nEnergyBins; ++iEnergyBin){
                    ...
                    if(i==2){
						...
						*/
			            ...
			            line = new TLine(0.2,-4000,0.2,4000);
    ...
    for(auto mapIt : trees){
        ...
        for(int entry = 0; entry < nEntries; ++entry){
			...
			if(anEvent->nClusters==1) {
		    	...
		    	float E_mean_post = 0.5*(anEvent->energyL_post + anEvent->energyR_post);
				energyBinAverage = FindBin(anEvent->energySum,ranges_doubleHits[index1])+1;
				enRatioBin = FindBin(E_mean/E_mean_post,enRatioBins)+1;
			}
			else if(anEvent->nClusters==3){	
				energyBinAverage=1;
			}
			...
			if(h2_tL_REFvsEL[index2]==NULL){ // time L/R - time REF vs energy
				...
				h2_tL_REFvsEL[index2] = new TH2F(Form("h2_tL_REFvsEL_%s",labelL.c_str()),"",500,0,1000,1000,-24000,24000);
				p1_tL_REFvsEL[index2] = new TProfile(Form("p1_tL_REFvsEL_%s",labelL.c_str()),"",50,0,1000);
				h2_tR_REFvsER[index2] = new TH2F(Form("h2_tR_REFvsER_%s",labelR.c_str()),"",500,0,1000,1000,-24000,24000);
				p1_tR_REFvsER[index2] = new TProfile(Form("p1_tR_REFvsER_%s",labelR.c_str()),"",50,0,1000);

				h2_tL_REFvsEL_pre[index2] = new TH2F(Form("h2_tL_REFvsEL_pre%s",labelL.c_str()),"",500,0,1000,1000,-24000,24000);
				p1_tL_REFvsEL_pre[index2] = new TProfile(Form("p1_tL_REFvsEL_pre%s",labelL.c_str()),"",50,0,1000);
				h2_tR_REFvsER_pre[index2] = new TH2F(Form("h2_tR_REFvsER_pre%s",labelR.c_str()),"",500,0,1000,1000,-24000,24000);
				p1_tR_REFvsER_pre[index2] = new TProfile(Form("p1_tR_REFvsER_pre%s",labelR.c_str()),"",50,0,1000);

				h2_tL_REFvsEL_post[index2] = new TH2F(Form("h2_tL_REFvsEL_post_%s",labelL.c_str()),"",500,0,1000,1000,-24000,24000);
				p1_tL_REFvsEL_post[index2] = new TProfile(Form("p1_tL_REFvsEL_post_%s",labelL.c_str()),"",50,0,1000);
				h2_tR_REFvsER_post[index2] = new TH2F(Form("h2_tR_REFvsER_post_%s",labelR.c_str()),"",500,0,1000,1000,-24000,24000);
				p1_tR_REFvsER_post[index2] = new TProfile(Form("p1_tR_REFvsER_post_%s",labelR.c_str()),"",50,0,1000);

				h2_tL_wREFvsEL[index2] = new TH2F(Form("h2_tL_wREFvsEL_%s",labelL.c_str()),"",500,0,1000,1000,-24000,24000);
				p1_tL_wREFvsEL[index2] = new TProfile(Form("p1_tL_wREFvsEL_%s",labelL.c_str()),"",80,0,1000);
				h2_tR_wREFvsER[index2] = new TH2F(Form("h2_tR_wREFvsER_%s",labelR.c_str()),"",500,0,1000,1000,-24000,24000);
				p1_tR_wREFvsER[index2] = new TProfile(Form("p1_tR_wREFvsER_%s",labelR.c_str()),"",80,0,1000);

				h2_tL_REFvsE_REF[index2] = new TH2F(Form("h2_tL_REFvsE_REF_%s",labelL.c_str()),"",500,0,450,1000,-24000,24000);
				h2_tR_REFvsE_REF[index2] = new TH2F(Form("h2_tR_REFvsE_REF_%s",labelR.c_str()),"",500,0,450,1000,-24000,24000);

				h2_tL_REFvsEL_corr[index2] = new TH2F(Form("h2_tL_REFvsEL_corr%s",labelL.c_str()),"",500,0,1000,1000,-24000,24000);
				h2_tR_REFvsER_corr[index2] = new TH2F(Form("h2_tR_REFvsER_corr%s",labelR.c_str()),"",500,0,1000,1000,-24000,24000);
				p1_tL_REFvsEL_corr[index2] = new TProfile(Form("p1_tL_REFvsEL_corr%s",labelL.c_str()),"",50,0,1000);
				p1_tR_REFvsER_corr[index2] = new TProfile(Form("p1_tR_REFvsER_corr%s",labelR.c_str()),"",50,0,1000);

				h2_tAve_REFvsE[index2] = new TH2F(Form("h2_tAve_REFvsE_%s",labelL.c_str()),"",500,0,1000,1000,-24000,24000);
				h2_tAve_REFvsE_corr[index2] = new TH2F(Form("h2_tAve_REFvsE_corr_%s",labelL.c_str()),"",500,0,1000,1000,-24000,24000);
				p1_tAve_REFvsE[index2] = new TProfile(Form("p1_tAve_REFvsE_%s",labelL.c_str()),"",50,0,1000);
				p1_tAve_REFvsE_corr[index2] = new TProfile(Form("p1_tAve_REFvsE_corr_%s",labelL.c_str()),"",50,0,1000);



			}

			if(h2_deltaT_vs_energyRatio_REF[index0]==NULL){ // delta T  vs energy Ratio of reference module 
				h2_deltaT_vs_energyRatio_REF[index0] = new TH2F(Form("h2_deltaT_vs_energyRatio_REF_Vov%.2f_th%02d",anEvent->Vov,anEvent->vth1),"",50,0,5,1000,-12000,12000);
				p1_deltaT_vs_energyRatio_REF[index0] = new TProfile(Form("p1_deltaT_vs_energyRatio_REF_Vov%.2f_th%02d",anEvent->Vov,anEvent->vth1),"",50,0,5);
			}

			/////////////////////////////////////////////////////////////
			/////////// Filling 3rd loop histograms /////////////////////
			/////////////////////////////////////////////////////////////
			...
			if(fabs(anEvent->timeL_ext - anEvent->timeR_ext)<10000 && anEvent->energyR_ext>0 && anEvent->energyL_ext>0){
				...
				h2_deltaT_vs_energyRatio_REF[index0] -> Fill(anEvent->energyR_ext/anEvent->energyL_ext, anEvent->timeL_ext - anEvent->timeR_ext);
				p1_deltaT_vs_energyRatio_REF[index0] -> Fill(anEvent->energyR_ext/anEvent->energyL_ext, anEvent->timeL_ext - anEvent->timeR_ext);
			}

	        if(fabs(deltaT)>10000) continue;
		    // std::cout << ">>> L3388 3rd loop DEBUG: entry=" << std::endl;
	        accepted3++;
            ...
            if(anEvent->nClusters==1 && anEvent->energyR_ext>0 && anEvent->energyL_ext>0){
				double time_ave=0.5*(static_cast<double>(anEvent->timeL)+static_cast<double>(anEvent->timeR));
				...
				if(time_ave - time_ave_REF >3000){
					h2_tAve_REFvsE[index2] -> Fill(0.5*(anEvent->energyL+anEvent->energyR), time_ave - time_ave_REF);
					p1_tAve_REFvsE[index2] -> Fill(0.5*(anEvent->energyL+anEvent->energyR), time_ave - time_ave_REF);
				}
			}
			if(anEvent->nClusters==2 && anEvent->energyL_post>0 && anEvent->energyR_post>0){
				if(anEvent->barID==maxBar) continue;
				...
				if(fabs(t_mean_a-t_mean_post_a)<10000 ){
					...
					if(!h1_deltaT_bars[index3]) continue;
					...
					//if((anEvent->energyR/anEvent->energyL > energyRatio_ranges[index2]->at(0)) && (anEvent->energyR/anEvent->energyL < energyRatio_ranges[index2]->at(1) ) &&
						...
						if(E_mean/E_mean_post>0.5 && E_mean/E_mean_post<2.){
							...
							h1_deltaT_bars[index3]->Fill(t_mean_a - t_mean_post_a - energyRatioCorr_bars);
						}
					//}			
				}
                // std::cout << ">>> L3462 3rd loop DEBUG: entry=" << std::endl;
    ...
    for(auto stepLabel : stepLabels){
		...
		latex -> SetTextColor(kRed);
		latex -> Draw("same");
		float fitXMin = fitFunc_energyRatio_REF[index0]->GetParameter(1) - 1.*fitFunc_energyRatio_REF[index0]->GetParameter(2);
		float fitXMax = fitFunc_energyRatio_REF[index0]->GetParameter(1) + 1.*fitFunc_energyRatio_REF[index0]->GetParameter(2);

		fitFunc_energyRatioCorrection_REF[index0] = new TF1(Form("fitFunc_energyRatioCorr_%s",extLabel.c_str()),"pol3",fitXMin,fitXMax);
		prof -> Fit(fitFunc_energyRatioCorrection_REF[index0],"QRS+");
		fitFunc_energyRatioCorrection_REF[index0] -> SetLineColor(kRed);
		fitFunc_energyRatioCorrection_REF[index0] -> SetLineWidth(2);
		fitFunc_energyRatioCorrection_REF[index0] -> Draw("same");
        ...
        for(int iBar = 0; iBar < 16; ++iBar){
			...
			int nEnRatioBins = enRatioBins->size()-1;
	        ...
	        for(int i=1;i<=4;i++){//loop over single double triple 
				for(int iEnergyBin = 1; iEnergyBin <= nEnergyBins; ++iEnergyBin) {
                    ...
                    if(vth1==15 && iBar>13) std::cout<<"drawing plots for: "<<index2<<std::endl;
					std::string labelLR_energyBin;
					std::string labelBars_energyBin;
					std::string labelBars_enRatioBin;
					if(i==1){labelLR_energyBin = Form("%s_energyBin%02d_S",labelLR.c_str(),iEnergyBin);}
					else if(i==2){
						labelLR_energyBin = Form("%s_energyBin%02d_D",labelLR.c_str(),iEnergyBin);
						labelBars_energyBin = Form("%s_energyBin%02d_D",labelBars.c_str(),iEnergyBin);
						for(int en=1;en<=nEnRatioBins+1;en++){
							labelBars_enRatioBin = Form("%s_enRatioBin%02d_enRatioBin%02d_D",labelBars.c_str(),iEnergyBin, en);
						}
					}
					else if(i==3){labelLR_energyBin = Form("%s_energyBin%02d_T",labelLR.c_str(),iEnergyBin);}
					...
					else{std::cout << "Skip fit: not enough entries for index2=" << index2 << " (entries=" << prof->GetEntries() << ")" << std::endl;}
					...
					delete latex;
                    if(i==1){
						c = new TCanvas(Form("c_deltaT_REF_raw_%s",labelLR_energyBin.c_str()),Form("c_deltaT_REF_raw_%s",labelLR_energyBin.c_str()));
						histo = h1_deltaT_raw_REF[index2];
					    histo -> SetLineWidth(2);
					    histo -> SetLineColor(kAzure);
					    histo -> SetMarkerColor(kAzure);
			            TF1* fitFunc = new TF1(Form("fitFunc_deltaT_raw_REF_%s",labelLR_energyBin.c_str()),"gaus",-10000, 10000);
					    drawDeltaT(c, histo, fitFunc, Form("(%02d - REF)  ",iBar), "raw","");
					    outFile -> cd();
					    histo -> Write();

						c -> Print(Form("%s/CTR_REF/1hit/c_deltaT_REF_raw_%s.png",plotDir.c_str(),labelLR_energyBin.c_str()));
				    	c -> Print(Form("%s/CTR_REF/1hit/c_deltaT_REF_raw_%s.pdf",plotDir.c_str(),labelLR_energyBin.c_str()));
						delete c;

						// --- draw timeAve-time_REF vs energy
						c = new TCanvas(Form("c_tAve_REFvsE_bar%02d_%s_S",iBar,stepLabel.c_str()),Form("c_tAve_REFvsE_bar%02d_%s_S",iBar,stepLabel.c_str()));
						h2 = h2_tAve_REFvsE[index2];
						int bx,by,bz;
						h2->GetMaximumBin(bx,by,bz);
						double xMax = h2->GetXaxis()->GetBinCenter(bx);
						xMax_h2_tAve_REFvsE[index2] = xMax;
						h2 -> GetYaxis()->SetRangeUser(3000, 8000);
                        h2 -> SetTitle(Form(";energy [a.u.]; time_{ave} - time_{REF} [ps]"));
						h2->Draw("colz");
						prof = p1_tAve_REFvsE[index2];
						prof -> SetMarkerSize(0.4);
						prof -> Draw("psame");
						fitFunc_timeAve_enCorr[index2] = new TF1(Form("fitFunc_timeAve_bar%02d_%s_S",iBar,stepLabel.c_str()),"[0]/x + [1]/(x*x) - [2]*x - [3]*x*x",ranges["L-R"][index1]->at(0),ranges["L-R"][index1]->at(1));
						prof->Fit(fitFunc_timeAve_enCorr[index2],"QRS");
						fitFunc_timeAve_enCorr[index2] -> SetLineColor(kRed);
						fitFunc_timeAve_enCorr[index2] -> SetLineWidth(2);
						fitFunc_timeAve_enCorr[index2] -> Draw("same");

						latex = new TLatex(0.40,0.8,Form("#splitline{bar %02d}{V_{OV} = %.2f V, th. = %d DAC}",iBar,Vov,int(vth1)));
					    latex -> SetNDC();
					    latex -> SetTextFont(42);
					    latex -> SetTextSize(0.04);
					    latex -> SetTextColor(kRed);
						...
						c->Print(Form("%s/timeCorrelation/c_tAve_REFvsE_bar%02d_%s_S.png",plotDir.c_str(),iBar,stepLabel.c_str()));
						c->Print(Form("%s/timeCorrelation/c_tAve_REFvsE_bar%02d_%s_S.pdf",plotDir.c_str(),iBar,stepLabel.c_str()));
						delete c;
						// --- profile tAve_REFvsE
						c = new TCanvas(Form("c_tAve_REFvsE_p_bar%02d_%s_S",iBar,stepLabel.c_str()),Form("c_tAve_REFvsE_p_bar%02d_%s_S",iBar,stepLabel.c_str()));
						prof -> GetYaxis()->SetRangeUser(3000, 8000);
						prof -> SetTitle(Form(";energy [a.u.]; time_{ave} - time_{REF} [ps]"));
						prof -> SetMarkerSize(0.4);
						prof -> Draw("psame");
						...
						c->Print(Form("%s/timeCorrelation/c_tAve_REFvsE_p_bar%02d_%s_S.png",plotDir.c_str(),iBar,stepLabel.c_str()));
						c->Print(Form("%s/timeCorrelation/c_tAve_REFvsE_p_bar%02d_%s_S.pdf",plotDir.c_str(),iBar,stepLabel.c_str()));
						
						delete latex;
						delete c;

						// --- draw timeL-time_REF vs energyL
						c = new TCanvas(Form("c_tL_REFvsEL_bar%02dL_%s_S",iBar,stepLabel.c_str()),Form("c_tL_REFvsEL_bar%02dL_%s_S",iBar,stepLabel.c_str()));
						...
						prof -> SetMarkerSize(0.4);
						prof -> Draw("psame");
						fitFunc_timeL[index2] = new TF1(Form("fitFunc_timeL_bar%02d_%s_S",iBar,stepLabel.c_str()),"[0]/x + [1]/(x*x) - [2]*x - [3]*x*x",ranges["L"][index1]->at(0),ranges["L"][index1]->at(1));
						...
						c->Print(Form("%s/timeCorrelation/c_tL_REFvsEL_bar%02dL_%s_S.png",plotDir.c_str(),iBar,stepLabel.c_str()));
						c->Print(Form("%s/timeCorrelation/c_tL_REFvsEL_bar%02dL_%s_S.pdf",plotDir.c_str(),iBar,stepLabel.c_str()));
						delete c;
						 // --- profile tL_eL
						c = new TCanvas(Form("c_tL_REFvsEL_p_bar%02dL_%s_S",iBar,stepLabel.c_str()),Form("c_tL_REFvsEL_p_bar%02dL_%s_S",iBar,stepLabel.c_str()));
						prof -> GetYaxis()->SetRangeUser(0, 10000);
						...
						line_h = new TLine(0,fitFunc->GetParameter(1),1000,fitFunc->GetParameter(1));
						line_h -> SetLineColor(kBlue);
						line_h -> SetLineWidth(2);
						line_h -> SetLineStyle(2);
						line_h -> Draw("same");
						...
						c->Print(Form("%s/timeCorrelation/c_tL_REFvsEL_p_bar%02dL_%s_S.png",plotDir.c_str(),iBar,stepLabel.c_str()));
						c->Print(Form("%s/timeCorrelation/c_tL_REFvsEL_p_bar%02dL_%s_S.pdf",plotDir.c_str(),iBar,stepLabel.c_str()));
						
						delete latex;
						delete c;

						// --- draw timeR-time_REF vs energyR
						c = new TCanvas(Form("c_tR_REFvsER_bar%02dR_%s_S",iBar,stepLabel.c_str()),Form("c_tR_REFvsER_bar%02dR_%s_S",iBar,stepLabel.c_str()));
						...
						fitFunc_timeR[index2] = new TF1(Form("fitFunc_timeR_bar%02dR_%s_S",iBar,stepLabel.c_str()),"[0]/x + [1]/(x*x) - [2]*x - [3]*x*x",ranges["R"][index1]->at(0),ranges["R"][index1]->at(1));
						...
						c->Print(Form("%s/timeCorrelation/c_tR_REFvsER_bar%02dR_%s_S.png",plotDir.c_str(),iBar,stepLabel.c_str()));
						c->Print(Form("%s/timeCorrelation/c_tR_REFvsER_bar%02dR_%s_S.pdf",plotDir.c_str(),iBar,stepLabel.c_str()));
						delete c;
                        
						// --- profile tR_eR
						c = new TCanvas(Form("c_tR_REFvsER_p_bar%02dR_%s_S",iBar,stepLabel.c_str()),Form("c_tR_REFvsER_p_bar%02dR_%s_S",iBar,stepLabel.c_str()));
						prof -> GetYaxis()->SetRangeUser(0, 10000);
						...
						//line_v -> Draw("same");
						//line_80 -> Draw("same");
						TLine* line_800 = new TLine(600,2500,600,10000);
						...
						c->Print(Form("%s/timeCorrelation/c_tR_REFvsER_p_bar%02dR_%s_S.png",plotDir.c_str(),iBar,stepLabel.c_str()));
						c->Print(Form("%s/timeCorrelation/c_tR_REFvsER_p_bar%02dR_%s_S.pdf",plotDir.c_str(),iBar,stepLabel.c_str()));
						delete line_h;
						delete line_v;
						//delete line_80;
						delete line_800;
                    ...
                    if(i==2){
						...
						TF1* fitFunc;
						for (int j=1; j<nEnRatioBins+1; j++){
							double index3 =  10000000000*j + index2; // enRatioBin=0 for single and triple hits
							c = new TCanvas(Form("c_deltaT_%s",labelBars_enRatioBin.c_str()),Form("c_deltaT_%s",labelBars_enRatioBin.c_str()));
							histo = h1_deltaT_bars[index3];
							histo -> SetLineWidth(2);
							histo -> SetLineColor(kBlue);
							histo -> SetMarkerColor(kBlue);
							fitFunc = new TF1(Form("fitFunc_deltaT_%s",labelBars_enRatioBin.c_str()),"gaus",-10000, 10000);
						...
						delete c;
                        if(vth1==15 && iBar>13) std::cout<<"line 2933: "<<index2<<std::endl;
    ...
    for(auto mapIt : trees){
        ...
        for(int entry = 0; entry < nEntries; ++entry){
			...
			if( h1_deltaT_energyRatioCorr[index2] == NULL ){
				...
				h2_deltaT_totRatioCorr_vs_totRatio[index2] = new TH2F(Form("h2_deltaT_totRatioCorr_vs_totRatio_%s",labelLR_energyBin.c_str()),"",50,fitFunc_totRatio[index2]->GetParameter(1)-3.*fitFunc_totRatio[index2]->GetParameter(2), fitFunc_totRatio[index2]->GetParameter(1)+3.*fitFunc_totRatio[index2]->GetParameter(2), 1000, -12000., 12000. );
				...
				h2_deltaT_energyRatioCorr_vs_t1fineMean[index2] = new TH2F(Form("h2_deltaT_energyRatioCorr_vs_t1fineMean_%s",labelLR_energyBin.c_str()),"",50,0,1000,1000, -12000., 12000.);
				h2_deltaT_totRatioCorr_vs_t1fineMean[index2] = new TH2F(Form("h2_deltaT_totRatioCorr_vs_t1fineMean_%s",labelLR_energyBin.c_str()),"",50,0,1000, 1000, -12000., 12000.);
				h2_deltaT_energyRatioCorr_totRatioCorr_vs_t1fineMean[index2] = new TH2F(Form("h2_deltaT_energyRatioCorr_totRatioCorr_vs_t1fineMean_%s",labelLR_energyBin.c_str()),"",50,0,1000, 1000, -12000., 12000.);
			}
			if(h1_deltaT_corr_REF[index2]==NULL){
                ...
                h1_deltaT_ave_corr_REF[index2] = new TH1F(Form("h1_deltaT_ave_corr_REF_%s",labelLR_energyBin.c_str()),"",2000,-12000,12000);
			...
			if(h1_deltaT_eneryRatioCorr_REF[index0]==NULL){
				h1_deltaT_eneryRatioCorr_REF[index0] = new TH1F(Form("h1_deltaT_energyRatioCorr_externalBar_Vov%.2f_th%02d",anEvent->Vov,anEvent->vth1),"",2000,-12000,12000);

				h2_deltaT_energyRatioCorr_vs_t1fineMean_REF[index0] = new TH2F(Form("h2_deltaT_energyRatioCorr_vs_t1fineMean_REF_Vov%.2f_th%02d",anEvent->Vov,anEvent->vth1),"",100,0,1000,2000,-12000,12000);
				p1_deltaT_energyRatioCorr_vs_t1fineMean_REF[index0] = new TProfile(Form("p1_deltaT_energyRatioCorr_vs_t1fineMean_REF_Vov%.2f_th%02d",anEvent->Vov,anEvent->vth1),"",50,0,1000);
			}
			float enRatioCorr_REF;
	        if(fitFunc_energyRatioCorrection_REF[index0]){
				enRatioCorr_REF = fitFunc_energyRatioCorrection_REF[index0]->Eval(anEvent->energyR_ext/anEvent->energyL_ext) - fitFunc_energyRatioCorrection_REF[index0]->Eval(fitFunc_energyRatio_REF[index0]->GetParameter(1)); 
			}
			long long deltaT_REF = anEvent->timeL_ext - anEvent->timeR_ext;
			float t1fineMean_REF = 0.5 * ( anEvent->t1fineR_ext + anEvent->t1fineL_ext );

            if(fabs(deltaT_REF - enRatioCorr_REF)<10000 && fitFunc_energyRatioCorrection_REF[index0]){
				float enMin = fitFunc_energyRatio_REF[index0]->GetParameter(1) - 1.*fitFunc_energyRatio_REF[index0]->GetParameter(2);
				float enMax = fitFunc_energyRatio_REF[index0]->GetParameter(1) + 1.*fitFunc_energyRatio_REF[index0]->GetParameter(2);
				if(anEvent->energyR_ext/anEvent->energyL_ext > enMin && anEvent->energyR_ext/anEvent->energyL_ext < enMax){
					h1_deltaT_eneryRatioCorr_REF[index0] -> Fill( deltaT_REF - enRatioCorr_REF );
				    h2_deltaT_energyRatioCorr_vs_t1fineMean_REF[index0] -> Fill( t1fineMean_REF, deltaT_REF - enRatioCorr_REF );
				    p1_deltaT_energyRatioCorr_vs_t1fineMean_REF[index0] -> Fill( t1fineMean_REF, deltaT_REF - enRatioCorr_REF );
				}	
			}
            ...
            if(anEvent->nClusters==1 && anEvent->energyL_ext>0 && anEvent->energyR_ext>0 && anEvent->barID!=maxBar){
				double tL_corr, tR_corr, time_ave;
				double time_ave_REF = 0.5*(static_cast<double>(anEvent->timeL_ext)+static_cast<double>(anEvent->timeR_ext));
				double xMax, corr;
				bool check_L = false , check_R = false;
				if((anEvent->timeL - time_ave_REF )> 3000 && anEvent->energyL> ranges["L"][index1]->at(0) && anEvent->energyL < ranges["L"][index1]->at(1)){
					...
					check_L = true;
					h2_tL_REFvsEL_corr[index2]-> Fill(anEvent->energyL, tL_corr - time_ave_REF);
					p1_tL_REFvsEL_corr[index2]-> Fill(anEvent->energyL, tL_corr - time_ave_REF);
				}
				if((anEvent->timeR - time_ave_REF )> 3000 && anEvent->energyR> ranges["R"][index1]->at(0) && anEvent->energyR < ranges["R"][index1]->at(1)){
					...
					check_R = true;
					h2_tR_REFvsER_corr[index2]-> Fill(anEvent->energyR, tR_corr - time_ave_REF);
					p1_tR_REFvsER_corr[index2]-> Fill(anEvent->energyR, tR_corr - time_ave_REF);
				}
				if(check_L && check_R){
				...
				double time_ave_raw = 0.5*(static_cast<double>(anEvent->timeL)+static_cast<double>(anEvent->timeR));
				if((time_ave_raw - time_ave_REF )>3000 && energy> ranges["L-R"][index1]->at(0) && anEvent->energyR < ranges["L-R"][index1]->at(1)){
					xMax = xMax_h2_tAve_REFvsE[index2];
					corr = fitFunc_timeAve_enCorr[index2]->Eval(anEvent->energyL) - fitFunc_timeL[index2]->Eval(xMax);
					time_ave = time_ave_raw - corr;
					h1_deltaT_ave_corr_REF[index2] -> Fill(time_ave - time_ave_REF);
					h2_tAve_REFvsE_corr[index2]-> Fill(energy, time_ave - time_ave_REF);
					p1_tAve_REFvsE_corr[index2]-> Fill(energy, time_ave - time_ave_REF);
				}
			...
			if(anEvent->nClusters==2 && anEvent->energyL_post>0 && anEvent->energyR_post>0){
				...
				if (fabs((t_mean_w-t_mean_post_w) - energyRatioCorr_bars_w)<10000 ){
					...
					if( ( t_mean_w-t_mean_post_w > my_timeLow ) && ( t_mean_w-t_mean_post_w < my_timeHig ) ){
						if(E_mean/E_mean_post>0.5 && E_mean/E_mean_post<2.){
							h1_deltaT_bars_w[index3]->Fill(t_mean_w - t_mean_post_w - energyRatioCorr_bars_w);
						}
					}
				}
    ...
    for(auto stepLabel : stepLabels) {
		...
		histo = h1_deltaT_eneryRatioCorr_REF[index0];
		histo -> SetLineWidth(2);
		...
		h2 = h2_deltaT_energyRatioCorr_vs_t1fineMean_REF[index0];
		h2 -> GetYaxis()->SetRangeUser(h2 -> GetMean(2) - 600., h2 -> GetMean(2)+ 600);
		h2 -> SetTitle(Form(";t1fineMean;#Deltat [ps]"));
		h2 -> Draw("colz");
		prof = p1_deltaT_energyRatioCorr_vs_t1fineMean_REF[index0];
        ...
        std::cout << "Drawn external bar plot 4th loop " << std::endl;
	        ...
	        for(int i=1;i<=3;i++){//loop over single double triple 
				for(int iEnergyBin = 1; iEnergyBin <= nEnergyBins; ++iEnergyBin){
					...
					if(i==1){
					    ...
					    drawDeltaT(c, histo, fitFunc, Form("(%02d - REF)  ",iBar), "en.Corr","");
					    outFile -> cd();
					    histo -> Write();

						c -> Print(Form("%s/CTR_REF/1hit/c_deltaT_REF_%s.png",plotDir.c_str(),labelLR_energyBin.c_str()));
				    	c -> Print(Form("%s/CTR_REF/1hit/c_deltaT_REF_%s.pdf",plotDir.c_str(),labelLR_energyBin.c_str()));
						delete c;

						c = new TCanvas(Form("c_tL_REFvsEL_corr_bar%02dL_%s_S",iBar,stepLabel.c_str()),Form("c_tL_REFvsEL_corr_bar%02dL_%s_S",iBar,stepLabel.c_str()));
						h2 = h2_tL_REFvsEL_corr[index2];
						h2 -> GetYaxis()->SetRangeUser(3000, 6000);
                        h2 -> SetTitle(Form(";energy_{L} [a.u.]; time^{corr}_{L} - time_{REF} [ps]"));
						h2->Draw("colz");
						prof = p1_tL_REFvsEL_corr[index2];
						prof -> SetMarkerSize(0.4);
						prof-> Draw("psame");
						latex = new TLatex(0.40,0.8,Form("#splitline{bar %02dL}{V_{OV} = %.2f V, th. = %d DAC}",iBar,Vov,int(vth1)));
					    latex -> SetNDC();
						...
						c->Print(Form("%s/timeCorrelation/c_tL_REFvsEL_corr_bar%02dL_%s_S.png",plotDir.c_str(),iBar,stepLabel.c_str()));
						c->Print(Form("%s/timeCorrelation/c_tL_REFvsEL_corr_bar%02dL_%s_S.pdf",plotDir.c_str(),iBar,stepLabel.c_str()));
						delete c;

						c = new TCanvas(Form("c_tR_REFvsER_corr_bar%02dR_%s_S",iBar,stepLabel.c_str()),Form("c_tR_REFvsER_corr_bar%02dR_%s_S",iBar,stepLabel.c_str()));
						h2 = h2_tR_REFvsER_corr[index2];
						h2 -> GetYaxis()->SetRangeUser(3000, 6000);
                        h2 -> SetTitle(Form(";energy_{R} [a.u.]; time^{corr}_{R} - time_{REF} [ps]"));
						h2->Draw("colz");
						prof = p1_tR_REFvsER_corr[index2];
						prof -> SetMarkerSize(0.4);
						prof-> Draw("psame");
						latex = new TLatex(0.40,0.8,Form("#splitline{bar %02dR}{V_{OV} = %.2f V, th. = %d DAC}",iBar,Vov,int(vth1)));
					    latex -> SetNDC();
						...
						c->Print(Form("%s/timeCorrelation/c_tR_REFvsER_corr_bar%02dR_%s_S.png",plotDir.c_str(),iBar,stepLabel.c_str()));
						c->Print(Form("%s/timeCorrelation/c_tR_REFvsER_corr_bar%02dR_%s_S.pdf",plotDir.c_str(),iBar,stepLabel.c_str()));
						delete c;

						c = new TCanvas(Form("c_deltaT(ave)_REF_%s",labelLR_energyBin.c_str()),Form("c_deltaT(ave)_REF_%s",labelLR_energyBin.c_str()));
						histo = h1_deltaT_ave_corr_REF[index2];
					    histo -> SetLineWidth(2);
					    histo -> SetLineColor(kRed+2);
					    histo -> SetMarkerColor(kRed+2);
			            fitFunc = new TF1(Form("fitFunc_deltaT(ave)_REF_%s",labelLR_energyBin.c_str()),"gaus",-10000, 10000);
					    drawDeltaT(c, histo, fitFunc, Form("(%02d - REF)  ",iBar), "en.Corr","");
					    outFile -> cd();
					    histo -> Write();

						c -> Print(Form("%s/CTR_REF/1hit/c_deltaT(ave)_REF_%s.png",plotDir.c_str(),labelLR_energyBin.c_str()));
				    	c -> Print(Form("%s/CTR_REF/1hit/c_deltaT(ave)_REF_%s.pdf",plotDir.c_str(),labelLR_energyBin.c_str()));
						delete c;
						c = new TCanvas(Form("c_tAve_REFvsE_corr_bar%02d_%s_S",iBar,stepLabel.c_str()),Form("c_tAve_REFvsE_corr_bar%02d_%s_S",iBar,stepLabel.c_str()));
						h2 = h2_tAve_REFvsE_corr[index2];
						h2 -> GetYaxis()->SetRangeUser(3000, 6000);
                        h2 -> SetTitle(Form(";energy [a.u.]; time^{corr}_{ave} - time_{REF} [ps]"));
						h2->Draw("colz");
						prof = p1_tAve_REFvsE_corr[index2];
						prof -> SetMarkerSize(0.4);
						prof-> Draw("psame");
						latex = new TLatex(0.40,0.8,Form("#splitline{bar %02d}{V_{OV} = %.2f V, th. = %d DAC}",iBar,Vov,int(vth1)));
					    latex -> SetNDC();
						...
						c->Print(Form("%s/timeCorrelation/c_tAve_REFvsE_corr_bar%02d_%s_S.png",plotDir.c_str(),iBar,stepLabel.c_str()));
						c->Print(Form("%s/timeCorrelation/c_tAve_REFvsE_corr_bar%02d_%s_S.pdf",plotDir.c_str(),iBar,stepLabel.c_str()));
						delete c;
					}
					if(i==2){
						// --- draw deltaT from bars difference with weigthed mean : start
						TF1* fitFunc;
						for (int enRatioBin=1; enRatioBin<=nEnRatioBins; ++enRatioBin){
							double index3( 10000000000*enRatioBin + (double)index1 );
							if(!h1_deltaT_bars_w[index3]) continue;
							labelBars_enRatioBin = Form("%s_energyBin%02d_enRatioBin%02d_D",labelBars.c_str(),iEnergyBin,enRatioBin);

							fitFunc = new TF1(Form("fitFunc_deltaT_bars_w_%s",labelBars_enRatioBin.c_str()),"gaus",-10000, 10000);
							c = new TCanvas(Form("c_deltaT_w_%s",labelBars_enRatioBin.c_str()),Form("c_deltaT_w_%s",labelBars_enRatioBin.c_str()));
							histo = h1_deltaT_bars_w[index3];
							histo -> SetLineWidth(2);
							...
							c -> Print(Form("%s/CTR_energyRatioCorr/c_deltaT__w_%s.png",plotDir.c_str(),labelBars_enRatioBin.c_str()));
							c -> Print(Form("%s/CTR_energyRatioCorr/c_deltaT__w_%s.pdf",plotDir.c_str(),labelBars_enRatioBin.c_str()));
							delete fitFunc;
							delete c;
						}
    ...
    for(auto mapIt : trees) {
        ...
        for(int entry = 0; entry < nEntries; ++entry) {
			...
			if(p1_deltaT_energyRatioCorr_vs_t1fineMean_REF[index0]){
				float t1fineMean_REF = 0.5* ( anEvent->t1fineR_ext + anEvent->t1fineL_ext );
	        	int t1fineBin_REF = p1_deltaT_energyRatioCorr_vs_t1fineMean_REF[index0]->FindBin(t1fineMean_REF);
	        	int t1fineBin2_REF = p1_deltaT_energyRatioCorr_vs_t1fineMean_REF[index0]->FindBin(h1_t1fineMean_REF[index0]->GetMean());
	        	float t1fineCorr_REF = p1_deltaT_energyRatioCorr_vs_t1fineMean_REF[index0]->GetBinContent(t1fineBin_REF) - p1_deltaT_energyRatioCorr_vs_t1fineMean_REF[index0]->GetBinContent( t1fineBin2_REF );
				enRatioCorr_REF = fitFunc_energyRatioCorrection_REF[index0]->Eval(anEvent->energyR_ext/anEvent->energyL_ext) - fitFunc_energyRatioCorrection_REF[index0]->Eval(fitFunc_energyRatio_REF[index0]->GetParameter(1)); 
			...
			if(h1_deltaT_eneryRatioCorr_pasheCorr_REF[index0]==NULL){
				h1_deltaT_eneryRatioCorr_pasheCorr_REF[index0] = new TH1F(Form("h1_deltaT_eneryRatioCorr_pasheCorr_REF_Vov%.2f_th%02d",anEvent->Vov,anEvent->vth1),"",2000,-12000.,12000.);
			}

			if(h2_deltaTave_vs_energyRatio[index2] == NULL){
				std::string label(Form("bars%02d-%02d_Vov%.2f_th%02d_%02dHits",anEvent->barID,anEvent->barID+1,anEvent->Vov,anEvent->vth1,energyBinAverage,anEvent->nClusters));
				p1_deltaTave_vs_energyRatio[index2] = new TProfile(Form("p1_deltaTave_vs_energyRatio_%s",label.c_str()),"",80,0,5.);
				h2_deltaTave_vs_energyRatio[index2] = new TH2F(Form("h2_deltaTave_vs_energyRatio_%s",label.c_str()),"",100,0.,5.,2000,-24000.,24000.);
			}
			...
			if(fabs(deltaT_REF - enRatioCorr_REF)<10000 && anEvent->energyL_ext>0 && anEvent->energyR_ext>0 && fitFunc_energyRatio_REF[index0]){
                float enMin = fitFunc_energyRatio_REF[index0]->GetParameter(1) - 1.*fitFunc_energyRatio_REF[index0]->GetParameter(2);
                float enMax = fitFunc_energyRatio_REF[index0]->GetParameter(1) + 1.*fitFunc_energyRatio_REF[index0]->GetParameter(2);
                if(anEvent->energyR_ext/anEvent->energyL_ext > enMin && anEvent->energyR_ext/anEvent->energyL_ext < enMax){
                    h1_deltaT_eneryRatioCorr_pasheCorr_REF[index0] ->Fill(deltaT_corr_REF);
                }	
            }
    ...
    for(auto stepLabel : stepLabels) {
		...
		histo = h1_deltaT_eneryRatioCorr_pasheCorr_REF[index0];
		histo -> SetLineWidth(2);
		histo -> SetLineColor(kGreen+2);
		histo -> SetMarkerColor(kGreen+2);
        ...
        for(int iBar = 0; iBar < 16; ++iBar) {
	        ...
	        int nEnergyBins = ranges["L-R"][index1]->size()-1;
			int nEnRatioBins = enRatioBins->size();
	        ...
	        for(int i=1; i<=3;i++){
				...
				for(int iEnergyBin = 1; iEnergyBin <= nEnergyBins; ++iEnergyBin) {
					...
					if(i==2){
						...
						std::cout << "Drawing deltaT between bars for " << labelLR_energyBin.c_str() << std::endl;
						...
						std::string labelLR_D(Form("%s_D",labelLR.c_str()));
						
						c = new TCanvas(Form("c_deltaTave_vs_enRatio_%s",labelLR_D.c_str()),Form("c_deltaTave_vs_enRatio_%s",labelLR_energyBin.c_str()));
						std::cout << "Drawing deltaTave vs energy ratio for " << labelLR_energyBin.c_str() << std::endl;
						...
						c -> Print(Form("%s/energyRatioCorr/c_deltaTave_vs_energyRatio__%s.pdf",plotDir.c_str(),labelLR_energyBin.c_str()));
						c -> Print(Form("%s/energyRatioCorr/c_deltaTave_vs_energyRatio__%s.png",plotDir.c_str(),labelLR_energyBin.c_str()));
						std::cout << "Saved canvas " << Form("%s/energyRatioCorr/c_deltaTave_vs_energyRatio__%s.pdf",plotDir.c_str(),labelLR_energyBin.c_str()) << std::endl;
						delete c;
						...
						c -> Print(Form("%s/energyRatioCorr/c_deltaTave_energyRatio_p__%s.pdf",plotDir.c_str(),labelLR_energyBin.c_str()));
						c -> Print(Form("%s/energyRatioCorr/c_deltaTave_energyRatio_p__%s.png",plotDir.c_str(),labelLR_energyBin.c_str()));
						delete latex;
						delete c;
					}
    ...
    //  if (useTrackInfo){
	std::map<double,TH1F*> h1_deltaTave_energyRatioCorr;
	std::map<double,TH1F*> h1_deltaTave_w_energyRatioCorr;
    for(auto mapIt : trees) {
        ModuleEventClass* anEvent = new ModuleEventClass();
        mapIt.second -> SetBranchAddress("event",&anEvent);
        ...
        for(int entry = 0; entry < nEntries; ++entry) {
		   ...
		   int enRatioBin=0;
	        ...
	        if(h1_deltaTave_energyRatioCorr[index3]==NULL){
				std::string labelLR_enBinRatio_bars(Form("bars%02d-%02d_Vov%.2f_th%02d_energyBin%02d_enBinRatio%02d_%02dHits",anEvent->barID,anEvent->barID+1,anEvent->Vov,anEvent->vth1,energyBinAverage,enRatioBin,anEvent->nClusters));
                h1_deltaTave_energyRatioCorr[index3] = new TH1F(Form("h1_deltaTave_energyRatioCorr_%s",labelLR_enBinRatio_bars.c_str()),"",2000,-12000.,12000.);
                h1_deltaTave_w_energyRatioCorr[index3] = new TH1F(Form("h1_deltaTave_w_energyRatioCorr_%s",labelLR_enBinRatio_bars.c_str()),"",2000,-12000.,12000.);
			}
			float deltaT_corr = deltaT  - energyRatioCorr - t1fineCorr;
			...
			if(anEvent->nClusters==2 && anEvent->energyL_post>0 && anEvent->energyR_post>0){
					...
					// if(0.5<E_mean/E_mean_post && E_mean/E_mean_post < 2.){
						h1_deltaTave_energyRatioCorr[index3] -> Fill(0.5*(deltaT_corr + deltaT_post_corr) - energyRatioCorr_bars_ave);
						h1_deltaTave_w_energyRatioCorr[index3] -> Fill((deltaT_corr*E_mean + deltaT_post_corr*E_mean_post)/(E_mean + E_mean_post) - energyRatioCorr_bars_ave);
					// }
			}
	        ...
	        for(int i=1;i<=3;i++){
				for(int iEnergyBin = 1; iEnergyBin <= nEnergyBins; ++iEnergyBin) {
					...
					if(i==2){
						...
						for (int enRatioBin=1; enRatioBin<=nEnRatioBins+1; ++enRatioBin){
							double index3(10000000000*enRatioBin + index2 );
							std::string labelLR_energyBin(Form("bars%02d-%02d_Vov%.2f_th%02d_energyBin%02d_enBinRatio%02d_%02dHits",iBar,iBar+1,Vov,int(vth1),iEnergyBin,enRatioBin,i));
							if(!h1_deltaTave_energyRatioCorr[index3]) continue;
							c = new TCanvas(Form("c_deltaTave_energyRatioCorr_%s",labelLR_energyBin.c_str()),Form("c_deltaTave_energyRatioCorr_%s",labelLR_energyBin.c_str()));
							histo = h1_deltaTave_energyRatioCorr[index3];
							histo -> SetLineWidth(2);
							histo -> SetLineColor(kGreen+1);
							histo -> SetMarkerColor(kGreen+1);
							...
							c -> Print(Form("%s/CTR_energyRatioCorr_phaseCorr/c_deltaTave_energyRatioCorr_%s.pdf",plotDir.c_str(),labelLR_energyBin.c_str()));
							c -> Print(Form("%s/CTR_energyRatioCorr_phaseCorr/c_deltaTave_energyRatioCorr_%s.png",plotDir.c_str(),labelLR_energyBin.c_str()));
							delete c;

							if(!h1_deltaTave_w_energyRatioCorr[index3]) continue;
							c = new TCanvas(Form("c_deltaTave_w_energyRatioCorr_%s",labelLR_energyBin.c_str()),Form("c_deltaTave_w_energyRatioCorr_%s",labelLR_energyBin.c_str()));
							histo = h1_deltaTave_w_energyRatioCorr[index3];
							histo -> SetLineWidth(2);
							histo -> SetLineColor(kGreen+1);
							histo -> SetMarkerColor(kGreen+1);
				
							fitFunc = new TF1(Form("fitFunc_energyCorr_ave_w_bars_%s",labelLR_energyBin.c_str()),"gaus",-10000, 10000);
							drawDeltaT(c, histo, fitFunc, Form("%02d - %02d average ",iBar,iBar+1), "allCorr","");
				
							outFile -> cd();
							histo -> Write();
									
							c -> Print(Form("%s/CTR_energyRatioCorr_phaseCorr/c_deltaTave_w_energyRatioCorr_%s.pdf",plotDir.c_str(),labelLR_energyBin.c_str()));
							c -> Print(Form("%s/CTR_energyRatioCorr_phaseCorr/c_deltaTave_w_energyRatioCorr_%s.png",plotDir.c_str(),labelLR_energyBin.c_str()));
							delete c;
						}
					}
