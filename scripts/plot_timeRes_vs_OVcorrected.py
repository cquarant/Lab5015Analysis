...
for d in file_dict:
    ...
    for j,filename in enumerate(d['file_list']):
        ...
        RMS_y = (sum((y - mean_y) ** 2 for y in y_values) / n) ** 0.5
        ...
        tgraph.SetPointError(j, 0, RMS_y)
...
for i, gr in enumerate(tGraphs.keys()):
    ...
    tGraphs[gr].SetLineColor(file_dict[i]['color'])

    tGraphs[gr].Draw("PL SAME")
    legend.AddEntry(tGraphs[gr], file_dict[i]['DM'], "pl")
